...
export function RarityTimeSeriesChart({ entries, timeRange }: RarityTimeSeriesChartProps) {
  const [chartData, setChartData] = useState<any>({ series: [], categories: [], timestamps: [] })
  const [isZoomed, setIsZoomed] = useState<boolean>(false)
  const chartRef = useRef<any>(null)

  // Process data for the chart
  useEffect(() => {
    const transformedData = transformRarityTimeSeriesData(entries)
  ...
  const options: ApexCharts.ApexOptions = {
    ...
    colors: chartData.series.map((s: any) => getRarityColor(s.name)),
    dataLabels: {
      enabled: false,
    },
    stroke: {
  ...
  return (
    <div className="h-[400px] w-full">
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-sm font-medium">Value by Rarity (Log Scale)</h3>
        {isZoomed && (
          <button
            onClick={resetZoom}
      ...
      <div className="text-xs text-muted-foreground mt-2">
        <p>Use the zoom tools or drag to select a region to zoom into a specific time period</p>
      </div>
    </div>
  )
}
